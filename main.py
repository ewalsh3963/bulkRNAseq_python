...
class CommandLine:
    ...
    def __init__(self, in_opts=None):
        ...
        subparsers = self.parser.add_subparsers(title="programs", 
                                                dest="program",
                                                metavar="[fullAlign, fastqDump, mkref, STAR, htseq, Normalzie, s3]")
        ## Full Pipeline
    ...
    def fullAlign_args(self):
        ...
        self.fullAlign_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-technology-usw2/Retrogenix/", type=str)
    ...
    def fastqDump_args(self):
        ...
        self.fastqDump_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-technology-usw2/Retrogenix/", type=str)
    ...
    def fastqQC_args(self):
        ...
        self.fastqQC_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-technology-usw2/Retrogenix/", type=str)
    ...
    def mkref_args(self):
        ...
        self.mkref_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-dev/Retrogenix/", type=str)
    ...
    def STAR_args(self):
        ...
        self.STAR_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-technology-usw2/Retrogenix/", type=str)
    ...
    def htseq_args(self):
        ...
        self.htseq_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-technology-usw2/Retrogenix/", type=str)
    ...
    def DownstreamAnalysis_args(self):
        ...
        self.DownstreamAnalysis_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-technology-usw2/Retrogenix/", type=str)
    ...
    def Normalize_args(self):
        ...
        self.Normalize_parse.add_argument('-s3', metavar='--parent_s3', action='store', help='S3 bucket name', required=False, default="s3://capsida-technology-usw2/Retrogenix/", type=str)
...
class LogInitializer:
    ...
    def logger_check(self):
        ...
        GEN_Tools.SearchDir.check_file(run_logs, critical=False, rewrite=True, force_rw=True)  # check logs file - create new log
        return run_logs
...
def main(command=None):
    ...
    if my_command.args.program == "fullAlign":
        ...
        s3_dir = os.path.join(my_command.args.s3, 'expdata', my_command.args.s)
        cmd = ['aws', 's3', 'sync', outdir, s3_dir]
        GEN_Tools.ProcessCall.command_process_background(cmd)
        ...
        ########### send to s3 ###########
        ## send fastq files to "expdata" folder in s3
        s3_dir = os.path.join(my_command.args.s3, 'genomes', my_command.args.s)
        cmd = ['aws', 's3', 'sync', outdir, s3_dir]
        GEN_Tools.ProcessCall.command_process_background(cmd)
        ...
        ########### send to s3 ###########
        ## send fastq files to "expdata" folder in s3
        s3_dir = os.path.join(my_command.args.s3, 'results', my_command.args.s)
        cmd = ['aws', 's3', 'sync', outdir, s3_dir]
        GEN_Tools.ProcessCall.command_process_background(cmd)
        ...
        ########### send to s3 ###########
        ## send fastq files to "expdata" folder in s3
        s3_dir = os.path.join(my_command.args.s3, 'Retrogenix_data', my_command.args.s)
        cmd = ['aws', 's3', 'sync', outdir, s3_dir]
        GEN_Tools.ProcessCall.command_process_background(cmd)
        ...
        ########### send to s3 ###########
        ## send fastq files to "expdata" folder in s3
        # s3_dir = os.path.join(my_command.args.s3, 'PublicRNAseqData', my_command.args.s)
        # cmd = ['aws', 's3', 'sync', outdir, s3_dir]
        # GEN_Tools.ProcessCall.command_process_background(cmd)

    elif my_command.args.program == 's3':
        ########### send to s3 ###########
        ## fastq files 
        local_dir = os.path.join('/ds-workspace/EW-TempDataStore', my_command.args.s, 'fastq')
        s3_dir = os.path.join(my_command.args.s3, 'expdata', my_command.args.s)
        
        ## send s3 fastq files to "expdata" folder
        # cmd = ['aws', 's3', 'sync', local_dir, s3_dir, '--no-progress']
        cmd = ['aws', 's3', 'sync', local_dir, s3_dir]
        GEN_Tools.ProcessCall.command_process_background(cmd)
...
# -s3 s3://capsida-technology-usw2/Retrogenix/

# python3 /captools/ewalsh/Retrogenix/FastqProcess/main.py fastqDump -s CHO_1_GSE138292 \
# -srr /captools/ewalsh/Retrogenix/FastqProcess/RunFiles/CHO_1_GSE138292/SraRunTable.csv \
# -s3 s3://capsida-technology-usw2/Retrogenix/ -N 1 -X 750000
